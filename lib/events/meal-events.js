...
const addMealTotals = () => {
  $($mealsCont).append(HtmlHelper.MealTotalsTable(mealTotals));
}
...
const removeFoodFromMeal = (e) => {
  ...
  const meal_id = e.target.closest('table').dataset.id;
  const meal = meals[meal_id - 1];
  const food_id = e.target.dataset.id;
...
$('#meals').on('click', '.meal-table .delete-food', removeFoodFromMeal);
